...
from database.models import Base, User, Schedule, Reminder, WorkerState
from database.operations import get_or_create_user, create_schedule
...
class TestDatabase:
    ...
    def test_worker_state(self, test_db):
        ...
        # create worker state
        state = WorkerState(
            worker_name="reminder_scheduler",
        ...
        # verify state
        found_state = test_db.query(WorkerState).filter_by(
            worker_name="reminder_scheduler"
        ).first()
...
class TestDataIntegrity:
    ...
    def test_required_fields(self):
        ...
        with pytest.raises(Exception):
            schedule = Schedule(peptide_name="Test")
